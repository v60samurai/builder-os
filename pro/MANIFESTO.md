...
- **Gear:** SCOPE HOLD
- **Artifact:** `prd/prd.md` (phase-split — Phase 1 = min usable), then `blueprint/blueprint-template.md` — the Blueprint (Part 1 is the engineering spec)
- **Allowed:** Writing the PRD (phase-split), classifying the build (see gate below), writing the Blueprint, prioritization calls, gate-checking with `prd-gate` + `blueprint-gate`.
...
- **Exit when:** `prd-gate` returns READY — non-goals real, guardrail metric exists, no placeholders, confidence tags on every claim; the build is classified; and for a build, `blueprint-gate` returns READY — the ⭐ structural decision resolved, no load-bearing 🔵, every chunk has boundary contracts.

### DELIVER
- **Purpose:** Build to spec. Ship.
- **Artifact:** the Blueprint's Part 3 — Build Plan (`blueprint/blueprint-template.md`, driven by `blueprint-runner`), building against the Blueprint's chunk map (Part 1); `brand/` guide (driven by `brand-guide-visualizer`)
...
- **Forbidden:** Re-opening DEFINE without a tripwire firing. A mid-build "actually, let's also add X" is scope creep unless a tripwire below justifies it. Starting a build before `blueprint-gate` is green.
...
- **Building in DELIVER but a core assumption is still 🔵 hypothesis** → drop back to DISCOVER. Shipping on an untested load-bearing assumption is the single most expensive mistake this manifesto exists to prevent. This includes *architectural* 🔵s — an unresolved ⭐ Blueprint decision is a load-bearing hypothesis; resolve or spike it before build.
- **"Build our own X" / "new service" / "new table"** → is there an existing pattern to mirror first? Reuse-don't-invent. New infrastructure needs a reason the existing pattern can't cover, stated in the Blueprint — not a default reached for because it's easier than reading the sibling code.
...
- **A session's done-check has a red item and the next session starts anyway** → stop. Per the playbook's own warning, this is the exact failure `blueprint-runner` exists to catch — a red item at Session 3 is cheap, the same gap at Session 8 is 3x the cost.
...
How a claim earns its confidence tag. Used in `discovery-brief.md`, `prd/prd.md`, `blueprint/blueprint-template.md`, and `postmortem-template.md`.
...
- **Build classification:** two axes, decided in DEFINE, stamped in the Blueprint header. **greenfield / extends-existing** — a new app (scaffold, schema-from-fixtures) vs a feature inside an existing repo (mirror a pattern source, spec deltas). **customer-facing / internal** — a product real users see (higher polish + design-system rigor) vs an internal tool (lighter, own conventions). Routes the whole build. Unstated → ask; don't assume.
- **Blueprint gate:** `blueprint-gate` refuses to pass until the **⭐ ONE structural decision is resolved (not 🔵)**, no 🔵 sits load-bearing on the critical path, every chunk has explicit boundary contracts, out-of-scope is stated, and auth/authz is declared per endpoint. This is the engineering half of the DEFINE→DELIVER gate — the PRD gate proves *what* is worth building; the Blueprint gate proves *how* is safe to start.
